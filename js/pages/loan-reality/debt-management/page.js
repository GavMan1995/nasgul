...
class DebtManagementPage extends Component {
  render () {
    const { location, reportItems, reportSections } = this.props
    const { accountId } = location.query
    const section = reportSections.filter((section) => section.code === 'debt_management')[0]
      ...
      <div className='o-site-container'>
        ...
        <div className='c-loan-reality-container'>
          ...
          {items.map((item, index) => {
            return <SectionCard key={index} item={item} />
          })}
        </div>
      </div>
    )
  ...
  componentDidMount () {
    const section = this.props.reportSections.filter((section) => {
      return section.code === 'debt_management'
    })[0]
...
module.exports = exports.default = connect(mapStateToProps)(AuthPage(DebtManagementPage))
module.exports.jsFilename = 'debt-management'
