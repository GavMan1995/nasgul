...
class CashManagementPage extends Component {
  render () {
    const { location, reportItems, reportSections } = this.props
    const { accountId } = location.query
    const section = reportSections.filter((section) => section.code === 'cash_management')[0]
      ...
      <div className='o-site-container'>
        ...
        <div className='c-loan-reality-container'>
          ...
          {items.map((item, index) => {
            return <SectionCard key={index} item={item} showTeaserText />
          })}
        </div>
      </div>
    )
  ...
  componentDidMount () {
    const section = this.props.reportSections.filter((section) => {
      return section.code === 'cash_management'
    })[0]
...
module.exports = exports.default = connect(mapStateToProps)(AuthPage(CashManagementPage))
module.exports.jsFilename = 'cash-management'
