...
class CustomerCountPage extends Component {
  render () {
    const { location, reportItems, reportSections } = this.props
    const { accountId } = location.query
    const section = reportSections.filter((section) => section.code === 'customer_mix')[0]
  ...
  componentDidMount () {
    const section = this.props.reportSections.filter((section) => {
      return section.code === 'customer_mix'
    })[0]
...
module.exports = exports.default = connect(mapStateToProps)(AuthPage(CustomerCountPage))
module.exports.jsFilename = 'customer-count'
